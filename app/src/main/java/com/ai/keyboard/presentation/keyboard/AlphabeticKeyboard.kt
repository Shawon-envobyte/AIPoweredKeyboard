...
import com.ai.keyboard.presentation.components.GestureWordPopup
import com.ai.keyboard.presentation.components.GlideTypingOverlay
...
fun AlphabeticKeyboard(
    ...
    Box(modifier = modifier.fillMaxWidth()) {
        Column(
                ...
                .pointerInput(Unit) {
                    ...
                    detectDragGestures(
                        onDragStart = { offset ->
                            ...
                            onIntent(KeyboardIntent.GlideTypingStarted(offset))
                        },
                        onDrag = { change, _ ->
                            // Use the absolute position from the change event
                            currentPosition = change.position
                            onIntent(KeyboardIntent.GlideTypingMoved(currentPosition))
                        },
                        onDragEnd = {
                            onIntent(KeyboardIntent.GlideTypingEnded)
                        }
                    )
                },
            ...
            if (isNumberRowEnabled) {
                KeyRow(
                    ...
                    onIntent = onIntent,
                    mode = mode,
                    glideTypingState = uiState.keyboardState.glideTypingState
                )
            }
            ...
            KeyRow(
                ...
                onIntent = onIntent,
                mode = mode,
                isGestureTypingActive = uiState.keyboardState.glideTypingState.isActive,
                glideTypingState = uiState.keyboardState.glideTypingState
            )
            ...
            Row(
                ...
                lowerCaseKeys[1].forEach { char ->
                    KeyButton(
                        ...
                        mode = mode,
                        isHighlighted = uiState.keyboardState.glideTypingState.highlightedKeys.contains(char),
                        isCurrentKey = uiState.keyboardState.glideTypingState.currentKey == char,
                        isGestureTypingActive = uiState.keyboardState.glideTypingState.isActive,
        ...
        // Glide typing overlay positioned absolutely over the keyboard
        if (uiState.keyboardState.glideTypingState.isActive) {
            GlideTypingOverlay(
                gesturePath = uiState.keyboardState.glideTypingState.gesturePath,
                isActive = uiState.keyboardState.glideTypingState.isActive,
        ...
        if (uiState.keyboardState.glideTypingState.isActive && 
            uiState.keyboardState.glideTypingState.gesturePath.isNotEmpty()) {
            GestureWordPopup(
                predictions = uiState.keyboardState.glideTypingState.predictions,
                currentPosition = uiState.keyboardState.glideTypingState.gesturePath.lastOrNull()?.position,
                isVisible = uiState.keyboardState.glideTypingState.isActive,
                modifier = Modifier.matchParentSize()
            )
        }
    }
